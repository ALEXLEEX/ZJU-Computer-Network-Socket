...
{
    int serverSocket = (long)(arg) & 0xffffffff; // Get server socket from argument.
...
void startSocketThread(int serverSocket)
{
    ...
    pthread_create(&thread, nullptr, Thread_Socket, (void *)(long)serverSocket);
}
...
{
    // Initialization.
    int serverSocket = init(SERVER_TCP);
...
{
    // Initialization.
    int serverSocket = init(SERVER_UDP);
    setOptions(serverSocket);
